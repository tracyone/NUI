...
CarPlay 式布局・负一屏与动态视频壁纸・高德地图深度联动・导航 / 巡航信息 HUD・天气与语音・音乐控制
...
* **右侧导航信息 HUD**（基于[高德车机版标准广播协议](docs/amap_auto_protocol.md)）：
...
* 左右滑动即可在「负一屏 → 桌面 → 应用列表」之间切换；应用列表分页浏览，常用、有图标的应用排在前面；长按图标可卸载 / 隐藏，也可进入**多选模式批量操作（支持本页全选）**，隐藏的应用可在设置中恢复

* **外观模式**：深色 / 浅色 / 跟随系统 / 跟随高德；白天 / 夜晚可分别设置壁纸，跟随外观自动切换，未设置时自动用另一张

* Dock 形态（悬浮圆角 / 贴边矩形）、图标大小、DPI 均可调

* **状态栏与系统导航栏可独立隐藏**，开启后界面自动为系统栏留出空间

### 🌌 负一屏



* 桌面最左侧的一页，**没有 Dock 栏**，画面更干净；在桌面地图页向右滑即可进入

* **负一屏壁纸可以和桌面不一样**，三种选择（桌面设置 → 外观 → 负一屏壁纸）：跟随桌面壁纸、单独选一张静态图片、或选一段**短视频循环播放**作为动态壁纸；无论图片还是视频都会铺满全屏、不留黑边

* **大号时间与日期**半透明显示（可在设置中关闭）；导航或巡航时也会悬浮显示实时导航信息；桌面歌词在负一屏同样可见

* **底部一条扁平、半透明的快捷栏**：回家 / 公司 / 收藏、音乐播放信息与控制、当前时间与日期，尽量把画面留给壁纸

* 可开启**闲置自动进入**：一段时间没有操作就自动切到负一屏（类似屏保），触摸即恢复；导航过程中不会误触发
...
![负一屏：独立壁纸、大号时钟、扁平底栏与桌面歌词](docs/images/minus-one.jpg)
...
* [高德车机版标准广播协议整理](docs/amap_auto_protocol.md) 与官方协议 PDF（`docs/`）：导航、巡航、红绿灯、摄像头、服务区、昼夜外观等广播字段
