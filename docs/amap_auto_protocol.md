...
| KEY_TYPE | 含义 | 关键字段 | 状态 |
|----------|------|----------|------|
| 10001 | 导航/巡航信息（核心） | `TYPE`：模式判据（0=GPS导航/1=模拟导航/2=巡航，见协议 PDF 第50页）<br>`CUR_SPEED`：当前速度<br>`LIMITED_SPEED`：限速<br>`ICON`/`NEW_ICON`：转向图标<br>`SEG_REMAIN_DIS_AUTO`：段剩余距离<br>`ROUTE_REMAIN_DIS`/`_AUTO`：全程剩余距离<br>`ROUTE_REMAIN_TIME_AUTO`：剩余时间<br>`ETA_TEXT`：预计到达<br>`NEXT_ROAD_NAME`/`CUR_ROAD_NAME`：下一条/当前道路名<br>`ROUTE_ALL_DIS`：全程总距离<br>`CAMERA_DIST`/`CAMERA_SPEED`：电子眼距离/限速<br>`endPOIName`：终点名称<br>`TRAFFIC_LIGHT_NUM`：红绿灯总数<br>`EXIT_NAME_INFO`/`EXIT_DIRECTION_INFO`：出口信息<br>`CAR_DIRECTION`：车头方向<br>**模式判定（对齐 Navi-Link 实测；TYPE 常不带、46/47 实车不可靠，均弃用）**：`ICON≠0`（NEW_ICON 优先，ICON 兜底）→导航模式（有转向引导）；`ICON=0`→巡航数据（非导航模式进入/更新巡航，导航模式忽略不打断）；退出巡航=10019 `STATE=25`；导航结束=10019 `STATE=9`（回桌面，等巡航数据重新进入） | ✅ 已实现 |
| 10019 | 高德状态 | `EXTRA_STATE`：8=导航中/9=导航结束/25=巡航结束/37=白天/38=夜晚/3=前台/4=后台 | ✅ 已实现（模式状态机+跟随地图外观） |
...
| 13011 | TMC 路况 | `EXTRA_TMC_SEGMENT`：JSON 路况分段数据（`tmc_segment_enabled` 是否显示、`tmc_segment_size` 分段数、`total_distance` 总路程、`residual_distance` 剩余总路程、`finish_distance` 已行驶里程、`tmc_info[]` 每段：`tmc_segment_number` 编号（越小越靠起点）/`tmc_status` 状态（-1无数据/0未知蓝/1畅通绿/2缓行黄/3拥堵红/4严重拥堵深红/10已驶过灰）/`tmc_segment_distance` 段距离（之和=剩余总路程）/`tmc_segment_percent` 占比）。**实测**：真实广播分段覆盖全程（段之和≈total_distance），段0=路线起点，需用 `finish_distance` 定位当前位置所在段再往后找（代码兼容两种坐标系：段和≈residual 时按剩余路程、否则按全程）。**导航状态下约 6s 一次** | ✅ 已实现（导航卡"前方拥堵"：距下一段拥堵实时距离，红=拥堵/深红=严重拥堵；status=10 已驶过不参与判断） |
